"""
Родительский класс для автотестирования модулей пакета UniflocPy на сходимость с Pipesim
...
class Autotest:
    ...
    @staticmethod
    def __calc_error_one_array(uniflocpy_array,
                               pipesim_array,
                               mean_integral_error):
        """
    ...
    def calc_error(self,
        ...
        if uniflocpy_results is not None and pipesim_results is not None:
            ...
            for key in pipesim_results.keys():
                error = self.__calc_error_one_array(uniflocpy_results[key], pipesim_results[key], mean_integral_error)
    ...
    def analyze_density_inversion(self,
                                  results):
        """
    ...
    def replace_num_flow_pattern(self,
                                 flow_pattern):
        """
    ...
    def define_pvt_table_data(self,
                              pipesim_results,
                              pars,
                              table_model_data):
        """
        ...
        if pipesim_results is not None:
            if calc_type.lower() == 'well':
                ...
                for i in range(len(pars)):
                    ...
                    # elif pars[i] == 'angle':
                    #     data = pipesim_results.profile[pipesim_results.cases[0]]
                    else:
                        data = pipesim_results.profile[pipesim_results.cases[0]][self.results_mapping[pars[i]]]
                ...
                for i in range(len(pars)):
                    ...
                    if pars[i] == 'esp_dp':
                        pipesim_par = uc.convert_pressure(pipesim_results.node[pipesim_results.cases[0]]
                                                          [self.results_mapping[pars[i]]]['Esp 1'], 'bar', 'pa')
                    elif pars[i] == 'q_mix':
                        ...
                        pipesim_par = uc.convert_pressure(pipesim_results.node[pipesim_results.cases[0]]
                                                          [self.results_mapping[pars[i]]]['Esp 1'], 'bar', 'pa')
                    elif pars[i] == 'esp_head':
                        pipesim_par = pipesim_results.node[pipesim_results.cases[0]][self.results_mapping[pars[i]]][
                            'Esp 1']

                    else:
                        pipesim_par = pipesim_results.profile[pipesim_results.cases[0]][self.results_mapping[pars[i]]]
    ...
    def save_results(self,
        ...
        if uniflocpy_results is not None:
            ...
            elif calc_type.lower() == 'well' and 'esp_system' in equipment_data:
                uniflocpy_results_df = pd.concat(uniflocpy_results.values(), axis=1)
            elif calc_type.lower() == 'well' and 'gl_system' in equipment_data:
                new_dict = {}
                for i, j in uniflocpy_results.items():
                    j = [j]
                    new_dict[i] = j
                # uniflocpy_results_df = pd.DataFrame.from_dict(new_dict)
                uniflocpy_results_df = pd.concat(uniflocpy_results.values(), axis=1)
            else:
        ...
        if pipesim_results is not None:
            if calc_type.lower() == 'pvt':
                ...
                pipesim_results_df = pd.concat(uniflocpy_results.values(), axis=1)
            elif calc_type.lower() == 'well' and 'gl_system' in equipment_data:
                new_dict = {}
                for i, j in uniflocpy_results.items():
                    j = [j]
                    new_dict[i] = j
                pipesim_results_df = pd.DataFrame.from_dict(new_dict)
            else:
    ...
    def pipesim_model(self,
        ...
        if 'gl_system' in equipment_data:
            ...
            if flow_direction == 1 and h_start == 'top':
                ...
                print('Пайпсим создает нагнетательную скважину')
