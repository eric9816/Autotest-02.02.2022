...
from autotest_pipe_subclass import AutotestPipe
from autotest_pvt_subclass import AutotestPVT
from autotest_gaslift_subclass import AutotestGasLift
from autotest_esp_subclass import AutotestESP
from autotest_choke_subclass import AutotestChoke
from autotest_network_subclass import AutotestNetwork
...
def calc_autotest(p_atma: float,
    ...
    if not calc_options['scenario']:
        if sample:  # Если sample = True, значит расчет моделей будет на произвольном наборе параметров
            ...
            if calc_type.lower() == 'pvt':  # тип расчета - PVT
            ...
            elif calc_type.lower() == 'network':
                network_sample = AutotestNetwork(model_path,
                                    file_path,
                                    trajectory_data,
                                    ambient_temperature_data,
                                    fluid_data,
                ...
                network_sample.sample_model(pars_limits=pars_limits,
                                  limited_pars=limited_pars,
                                  number_of_samples=number_of_samples,
                                  pfl=p_atma,
                                  model_path=model_path,
            ...
            if calc_type.lower() == 'pvt':
                ...
                network = AutotestNetwork(model_path,
                                        file_path,
                                        trajectory_data,
                                        ambient_temperature_data,
                                        fluid_data,
                ...
                network.main(fluid_data=fluid_data,
                             pipe_data=data,
                             pfl=p_atma,
                             model_path=model_path,
                             calc_options=calc_options,
